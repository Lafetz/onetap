...
func (q *Queries) GetTierLevel(ctx context.Context, arg GetTierLevelParams) (GetTierLevelRow, error) {
	row := q.db.QueryRowContext(ctx, GetTierLevel, arg.MerchantID, arg.Name)
	var i GetTierLevelRow
	err := row.Scan(
		&i.MerchantID,
		&i.Name,
		&i.MinPoints,
...
func (q *Queries) ListTierLevels(ctx context.Context, merchantID uuid.UUID) ([]ListTierLevelsRow, error) {
	rows, err := q.db.QueryContext(ctx, ListTierLevels, merchantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListTierLevelsRow
	for rows.Next() {
		var i ListTierLevelsRow
		if err := rows.Scan(
			&i.MerchantID,
			&i.Name,
			&i.MinPoints,
